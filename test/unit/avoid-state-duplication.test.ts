...
describe('avoidStateDuplicationRule', () => {
  ...
  it('should detect state initialized from props', () => {
    ...
    const propsInitIssues = issues.filter((i) => i.message.includes('initialized from prop'));
  ...
  it('should detect state duplicating other state', () => {
    ...
    const duplicationIssues = issues.filter(
      (i) => i.message.includes('is initialized from') && i.message.includes('user.')
    );
    ...
    const setterDuplication = issues.filter((i) => i.message.includes('being set from state'));
  ...
  it('should detect storing full objects when only using parts', () => {
    ...
    const selectiveIssues = issues.filter(
      (i) => i.message.includes('stores') && i.message.includes('properties but only uses')
    );
  ...
  it('should not flag direct prop usage', () => {
    ...
    const goodPropsIssues = issues.filter((i) => i.message.includes('GoodPropsUsage'));
  ...
  it('should not flag properly derived state', () => {
    ...
    const derivedStateIssues = issues.filter(
      (i) => i.message.includes('userDetails') && i.message.includes('initialized from prop')
    );
  ...
  it('should not flag single source of truth patterns', () => {
    ...
    const singleSourceIssues = issues.filter(
      (i) => i.message.includes('selectedUserId') && i.line > 90 // GoodSingleSource is later in the file
    );
