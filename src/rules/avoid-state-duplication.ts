...
function extractComponentProps(path: NodePath): string[] {
  const props: string[] = [];

  // Check function parameters for props
  const functionNode = path.node as t.Function;
...
function checkStateDuplication(
  ...
  path.traverse({
    CallExpression(callPath: NodePath<t.CallExpression>) {
      ...
      const setterState = stateCalls.find(
        (s) => t.isIdentifier(node.callee) && node.callee.name === s.setterName
      );
      ...
      if (setterState && node.arguments.length > 0) {
        ...
        const sourceState = stateCalls.find((s) => t.isIdentifier(arg) && arg.name === s.name);
